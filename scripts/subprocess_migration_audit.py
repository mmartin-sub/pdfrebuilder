...
class SubprocessUsageAnalyzer:
    ...
    def scan_codebase(self) -> dict:
        ...
        return {
            ...
            "files_with_subprocess": len([f for f in all_findings if f.get("ast_findings") or f.get("regex_findings")]),
    ...
    def _categorize_findings(self, all_findings: list[dict]):
        ...
        for file_result in all_findings:
            ...
            for finding in file_result.get("ast_findings", []) + file_result.get("regex_findings", []):
                ...
                self.findings[f"{highest_risk}_risk"].append(
                    {
                        ...
                        "findings": file_result.get("ast_findings", []) + file_result.get("regex_findings", []),
                    }
                )
    ...
    def _create_migration_priority(self) -> list[dict]:
        ...
        for item in self.findings["medium_risk"]:
            call_count = len([f for f in item.get("findings", []) if f.get("type") == "subprocess_call"])
            if call_count > 3:
                priority_list.append(
                    {
                        "priority": 2,
        ...
        nosec_files = {item["file"] for item in self.findings.get("nosec_suppressions", [])}
...
def main():
    ...
    print(f"   Already secure files: {len(results['findings_by_risk']['already_secure'])}")
    ...
    for i, item in enumerate(results["migration_priority"][:10], 1):
        ...
        print(f"      Effort: {item['migration_effort']} | Impact: {item['security_impact']}")
        print()
