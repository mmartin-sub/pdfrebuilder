...
def test_python_api() -> dict[str, Any]:
    ...
    try:
        ...
        if result.modified_elements == 2:
            ...
            print(f"  ❌ Batch text replacement: FAILED (expected 2, got {result.modified_elements})")

        # Test variable substitution
        variables = [
            VariableSubstitution(variable_name="COMPANY_NAME", replacement_value="Acme Corp"),
            VariableSubstitution(variable_name="INVOICE_NUMBER", replacement_value="INV-001"),
        ]
        ...
        if result.modified_elements == 2:
            ...
            print(f"  ❌ Variable substitution: FAILED (expected 2, got {result.modified_elements})")
    ...
    try:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print(f"  ❌ Substitute command: FAILED (return code: {result.returncode})")
            ...
            if result.returncode == 0:
                ...
                print(f"  ❌ Replace command: FAILED (return code: {result.returncode})")
                print(f"    Error: {result.stderr}")
            ...
            if result.returncode == 0:
                ...
                print(f"  ❌ Validate command: FAILED (return code: {result.returncode})")
                print(f"    Error: {result.stderr}")
...
def test_example_script() -> dict[str, Any]:
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            if "Total text elements: 4" in output and "Elements with variables: 4" in output:
