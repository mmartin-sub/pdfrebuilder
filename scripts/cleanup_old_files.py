    ...
    def __init__(
        ...
        try:
            ...
            if font_validation_demo_reports_dir:
                self.never_delete_patterns.append(os.path.basename(font_validation_demo_reports_dir))
        except ImportError:
    ...
    def scan_directory(self, directory: Path) -> list[tuple[Path, datetime]]:
        ...
        try:
            ...
            for item in items:
                ...
                if not is_safe:
                    logger.debug(f"File checked: {item} - Safe to delete: False, Protected by NEVER_DELETE patterns")
                    continue

                # Check age
                file_age = self.get_file_age(item)
                if file_age is None:
                    logger.debug(f"File checked: {item} - Safe to delete: False, No age information")
                    continue
                ...
                if is_old:
                    logger.debug(f"File checked: {item} - Safe to delete: True, Age: {age_days} days, CANDIDATE")
                    candidates.append((item, file_age))
                else:
                    logger.debug(f"File checked: {item} - Safe to delete: False, Age: {age_days} days, Too recent")
    ...
    def cleanup_files(self) -> tuple[int, int]:
        ...
        for file_path, mod_time in all_candidates:
            try:
                ...
                if self.dry_run:
                    print(f"Would delete: {file_path} (age: {age_days} days, size: {file_size} bytes)")
                    files_deleted += 1
                    ...
                    files_deleted += 1
                    total_size += file_size
                    print(f"Deleted: {file_path} (age: {age_days} days, size: {file_size} bytes)")
                else:
