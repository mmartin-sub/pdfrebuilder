...
@click.option(
    "--input-file", required=True, type=click.Path(exists=True, dir_okay=False), help="Path to the input document."
)
@click.option("--output-file", required=True, type=click.Path(dir_okay=False), help="Path for the output file.")
@click.option(
    "--input-engine", default="auto", help="The input engine to use (e.g., 'fitz', 'wand'). Defaults to 'auto'."
)
@click.option(
    "--output-engine", required=True, help="The output engine to use (e.g., 'wand_image', 'reportlab', 'krita')."
)
def main(input_file, output_file, input_engine, output_engine):
    ...
    try:
        ...
        if output_engine in ["reportlab", "fitz", "pymupdf"]:
            ...
            click.echo(f"...Loaded '{output_engine}'.")
            # Step 3: Render the document
            click.echo("\nStep 3: Rendering output document...")
            output_dir = os.path.dirname(output_file)
            renderer.render(input_file, output_dir)
