    ...
    for engine in engines:
        ...
        try:
            ...
            results[engine] = {
                ...
                "total_elements": sum(
                    len(layer.content) for page in document.document_structure for layer in page.layers
                ),
            }
...
def performance_comparison(file_path: str):
    ...
    if successful_engines:
        fastest_input = min(successful_engines.keys(), key=lambda k: successful_engines[k]["parse_time"])
    ...
    for engine, result in input_results.items():
        if result["success"]:
            print(f"  {engine}: {result['parse_time']:.2f}s, {result['total_elements']} elements")
        else:
