...
class BatchProcessingResult:
    ...
    def get_summary(self) -> dict:
        ...
        return {
            ...
            "average_processing_time": (self.processing_time / self.total_files if self.total_files > 0 else 0),
            "errors": self.errors,
        }
...
class BatchProcessor:
    ...
    def process_files_sequential(self, file_paths: list, processor_func, **kwargs) -> BatchProcessingResult:
    ...
    def process_files_parallel(self, file_paths: list, processor_func, **kwargs) -> BatchProcessingResult:
...
def pdf_extraction_processor(file_path: Path, output_dir: Path) -> dict:
    ...
    metadata = {
        ...
        "pages": (len(document.document_structure) if hasattr(document, "document_structure") else 0),
...
def pdf_reconstruction_processor(file_path: Path, output_dir: Path) -> dict:
    ...
    metadata = {
        ...
        "pages": (len(config_data.get("document_structure", [])) if isinstance(config_data, dict) else 0),
    }
...
def example_batch_pdf_extraction():
    ...
    print("\n--- Sequential Processing ---")
    sequential_result = processor.process_files_sequential(pdf_files, pdf_extraction_processor, output_dir=output_dir)

    # Parallel processing (if more than 1 file)
    if len(pdf_files) > 1:
        print("\n--- Parallel Processing ---")
        parallel_result = processor.process_files_parallel(pdf_files, pdf_extraction_processor, output_dir=output_dir)

        # Compare performance
        print("\nPerformance Comparison:")
        print(f"  Sequential: {sequential_result.processing_time:.2f}s")
        print(f"  Parallel:   {parallel_result.processing_time:.2f}s")
        print(f"  Speedup:    {sequential_result.processing_time / parallel_result.processing_time:.2f}x")
    ...
    for result in sequential_result.results:
        if result["success"] and result["metadata"]:
            ...
            print(f"   {Path(result['file_path']).name}: {metadata['pages']} pages, {metadata['file_size']:,} bytes")
...
def example_batch_variable_substitution():
    ...
    # Summary
    successful = sum(1 for r in results if r["success"])
    total_substitutions = sum(r.get("substitutions", 0) for r in results if r["success"])
...
def example_batch_format_conversion():
    ...
    result = processor.process_files_parallel(json_files, conversion_processor, output_dir=output_dir)
