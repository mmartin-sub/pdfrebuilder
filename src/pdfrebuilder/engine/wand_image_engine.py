...
class WandImageOutputEngine(DocumentRenderer):
    ...
    def render(self, document: UniversalDocument, output_path: str, options: dict | None = None):
        """
        ...
        if output_format not in self.supported_features:
            ...
            raise RenderingError(
                f"Unsupported image format: {output_format}. Supported formats: {self.supported_features}"
            )
                    ...
                    if isinstance(element, TextElement):
                        ...
                        if element.font_details.color:
                            ...
                            draw.fill_color = Color(
                                f"rgba({color.r * 255}, {color.g * 255}, {color.b * 255}, {color.a})"
                            )
                        ...
                        with Image(filename=element.image_file) as elem_img:
                            img.composite(
                                elem_img,
                                left=int(element.bbox.x1),
                                top=int(element.bbox.y1),
                            )
