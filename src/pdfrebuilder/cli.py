#!/usr/bin/env python3
"""CLI entry point for pdfrebuilder."""
...
def run_pipeline(args):
    ...
    # Import the pipeline functionality from main.py
    ...
    # Use configuration values, with CLI args taking precedence
    extraction_flags = {
        "include_text": getattr(args, "extract_text", True),
        "include_images": getattr(args, "extract_images", True),
        "include_drawings_non_background": getattr(args, "extract_drawings", True),
        "include_raw_background_drawings": getattr(args, "extract_raw_backgrounds", False),
    }
    ...
    try:
        ...
        if args.mode == "extract":
            ...
            console_print(f"Using input engine: {getattr(args, 'input_engine', 'auto')}", "info")
            # Log engine version info at DEBUG level if using fitz
            if getattr(args, "input_engine", "auto").lower() in ["fitz", "auto"]:
                console_print(
                    f"Input engine selected: {getattr(args, 'input_engine', 'auto')}",
                    "debug",
                    logging.DEBUG,
                )
            try:
                content = parse_document(
                    args.input,
                    extraction_flags,
                    engine=getattr(args, "input_engine", "auto"),
                )
            ...
            cli_args = {"output_engine": getattr(args, "output_engine", "auto")}

            # Load complete engine configuration
            engine_config = load_engine_config(cli_args=cli_args)

            engine_name = getattr(args, "output_engine", "auto")
            if engine_name == "auto":
            ...
            try:
                ...
                engine.generate(config_data, args.output, original_file)
            ...
            try:
                console_print(f"Parsing document: {args.input}", "info")
                console_print(
                    f"Using input engine: {getattr(args, 'input_engine', 'auto')}",
                    "info",
                )
                # Log engine version info at DEBUG level if using fitz
                if getattr(args, "input_engine", "auto").lower() in ["fitz", "auto"]:
