                ...
                "layers": [
                    {
                        ...
                        "content": [
                            ...
                            },
                            {
                                "type": "text",
                                "id": "text_3",
                                "text": "Text with special characters: ñáéíóú",
                                "font_details": {
                                    "name": "Times-Roman",
                                    "size": 12,
                                    "color": 0,
...
def demonstrate_font_validation():
    ...
    print("🔤 Font Validation System Demonstration")
    print("=" * 50)
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        print(f"📁 Using temporary fonts directory: {fonts_dir}")
        ...
        print(f"📄 Created sample layout config: {config_path}")

        # Initialize font validator
        font_validator = FontValidator(fonts_dir)
        set_font_validator(font_validator)

        print(f"🔍 Available fonts in directory: {len(font_validator.available_fonts)}")

        # Perform font validation
        print("\n🔍 Performing font validation...")
        validation_result = font_validator.validate_document_fonts(layout_config)

        # Display validation results
        print("\n📊 Font Validation Results:")
        print(f"  • Fonts required: {len(validation_result.fonts_required)}")
        print(f"  • Fonts available: {len(validation_result.fonts_available)}")
        print(f"  • Fonts missing: {len(validation_result.fonts_missing)}")
        print(f"  • Font substitutions: {len(validation_result.fonts_substituted)}")
        print(f"  • Coverage issues: {len(validation_result.font_coverage_issues)}")
        print(f"  • Validation passed: {validation_result.validation_passed}")

        # Show detailed information
        if validation_result.fonts_required:
            print(f"\n📝 Required fonts: {', '.join(validation_result.fonts_required)}")

        if validation_result.fonts_available:
            print(f"✅ Available fonts: {', '.join(validation_result.fonts_available)}")

        if validation_result.fonts_missing:
            print(f"❌ Missing fonts: {', '.join(validation_result.fonts_missing)}")

        # Simulate font substitutions
        print("\n🔄 Simulating font substitutions...")
        ...
        print(f"📝 Tracked {len(font_validator.substitution_tracker)} font substitutions")

        # Show substitution details
        for i, sub in enumerate(font_validator.substitution_tracker, 1):
            print(f"  {i}. {sub.original_font} → {sub.substituted_font} ({sub.reason})")
            if sub.element_id:
                print(
        ...
        print("\n📄 Generating font validation report...")
        ...
        print("📊 Generated validation reports:")
        ...
        if "json" in report_paths and os.path.exists(report_paths["json"]):
            ...
            if "metadata" in report_data and "font_validation" in report_data["metadata"]:
                ...
                print(f"  • Fonts required: {len(font_data.get('fonts_required', []))}")
                print(f"  • Fonts missing: {len(font_data.get('fonts_missing', []))}")
                print(f"  • Substitutions: {len(font_data.get('fonts_substituted', []))}")

        print("\n✅ Font validation demonstration completed!")
        ...
        print(f"📁 Reports copied to: {os.path.abspath(permanent_dir)}")
