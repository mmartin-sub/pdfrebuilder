...
class TestGoogleFontsAPI(unittest.TestCase):
    ...
    def test_css_fetch_http_error(self, mock_get):
        ...
        with patch("src.font.googlefonts.logging") as mock_logging:
            result = download_google_font(font_family, self.temp_dir)
    ...
    def test_default_destination_directory(self):
        ...
        with patch("src.font.googlefonts.requests.get") as mock_get:
            css_response = Mock()
            css_response.status_code = 200
            css_response.text = """
            ...
            with patch("src.font.googlefonts.get_config_value", return_value="./output/fonts"):
